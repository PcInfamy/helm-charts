...
LibreNMS automatically generates a secure Laravel APP_KEY on first run and stores it in a Kubernetes Secret for persistent reuse across pod restarts and upgrades.
...
| librenms.appkey | string | `""` | Optional user-supplied APP_KEY. If empty, Helm auto-generates a random key on first install and persists it in a Kubernetes Secret for reuse on upgrades. Format: base64:BASE64_ENCODED_32_BYTES (generate with: php artisan key:generate --show) |
| librenms.appkeySecretName | string | `""` | Optional override for the name of the auto-managed APP_KEY Secret. If not specified, defaults to "<release-name>-appkey". Only used when existingSecret is empty. |
...
| librenms.existingSecret | string | `""` | If set, the chart will reference this existing Secret instead of creating one. The Secret must contain a key named "appkey" with the base64-encoded APP_KEY value. Leave empty to let the chart manage the Secret automatically. |
